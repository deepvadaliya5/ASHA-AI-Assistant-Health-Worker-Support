 ...
 * ─────────────────────────────────────────────────────
 * GenAI Features:
 *  1.  Health Gatekeeper         — OpenAI intent classifier
 *  2.  Dynamic Web Mining        — Live Google News RSS per symptom
 *  3.  Semantic Ranking          — OpenAI embeddings + cosine similarity
 *  4.  Action-First AI Response  — Structured OpenAI chat prompt
 *  5.  Multi-turn Memory         — Full conversation history sent to OpenAI
 *  6.  Vision                    — Photo symptom analysis (base64 image)
 *  7.  AI Triage Scoring         — 0-100 urgency score per consultation
 *  8.  Smart Follow-up Questions — OpenAI generates 3 contextual next questions
 *  9.  AI Treatment Plan         — Step-by-step home care + referral plan
 * 10.  Outbreak Detector         — OpenAI analyses log clusters for alerts
 * 11.  Voice Transcription       — Whisper when Web Speech fails
...
try {
  ...
  console.log("[ENV] No .env — using system env");
}
...
// NOTE: express.static is mounted after all /api/* routes so POST /api/chat is never
// answered with 405 from the static file middleware.
...
async function readLogs() {
  try {
    return JSON.parse(await fs.readFile(LOGS_FILE, "utf8"));
  } catch {
    return [];
  }
}
...
async function chatCompleteText(messages, opts = {}) {
  const {
    model = CHAT_MODEL,
    temperature = 0.35,
    max_tokens = 4096,
...
/** Map client history to OpenAI roles; avoid duplicating the current user turn. */
function historyToOpenAIMessages(history, query) {
  const slice = history.slice(-6);
  const out = [];
  for (const h of slice) {
    const role = h.role === "assistant" ? "assistant" : "user";
    out.push({ role, content: String(h.content ?? "") });
  }
  const last = out[out.length - 1];
  const q = String(query || "").trim();
  if (!last || last.role !== "user" || last.content.trim() !== q) {
...
Respond with ONLY one line of raw JSON: {"isHealth": true, "confidence": 0.95}`;
  ...
  try {
    const raw = await chatCompleteText([{ role: "user", content: prompt }], {
      temperature: 0.1,
    });
...
async function mineHealthArticles(symptoms) {
  const q = encodeURIComponent(
    symptoms.join(" ") + " India health treatment 2024",
  );
...
async function semanticRank(query, articles) {
  ...
  const scored = await Promise.all(
    articles.map(async (a) => ({
      ...a,
      score: cosine(qv, await embed(`${a.title} ${a.snippet}`)),
    })),
  );
...
async function generateASHAResponse(
  query,
  symptoms,
  articles,
  language,
...
RULES: Use • bullets not asterisks. Numbered lists for actions. If fever 3+ days → add FEVER ALERT at top. Simple field-ready language.`;
  ...
  const userThread = historyToOpenAIMessages(history, query);
  const messages = [
  ...
  try {
    return await chatCompleteText(messages, { temperature: 0.4 });
  } catch (e) {
    console.warn("[ASHA Response] fallback:", e.message);
    return await chatCompleteText(
      [
        { role: "system", content: systemPrompt },
...
score: 0-100 (100=life-threatening). level: "critical"|"high"|"medium"|"low"`;
  ...
  try {
    ...
    return { score: 50, level: "medium", reasoning: "Assessment unavailable" };
  }
}
...
app.post("/api/treatment-plan", async (req, res) => {
  ...
  if (!symptoms.length && !diagnosis)
    return res.status(400).json({ error: "Symptoms or diagnosis required." });
...
Return ONLY raw JSON:
{
  ...
  try {
    ...
    return res.json(parseModelJSON(raw));
  } catch (e) {
    return res.status(500).json({ error: e.message });
  }
});

// ─── 6. VISION — PHOTO SYMPTOM ANALYSIS ───────────────────────────────────────
...
Return ONLY raw JSON:
{
  ...
  try {
    ...
    return res.json(parseModelJSON(out));
  } catch (e) {
    return res
      .status(500)
      .json({ error: "Image analysis failed: " + e.message });
  }
});

// ─── 11. WHISPER VOICE TRANSCRIPTION FALLBACK ─────────────────────────────────
app.post("/api/transcribe", async (req, res) => {
  ...
  try {
    ...
    if (buf.length < 64)
      return res.status(400).json({ error: "Audio too short or invalid." });

    let ext = "webm";
    if (String(mimeType).includes("ogg")) ext = "ogg";
    else if (String(mimeType).includes("mp4") || String(mimeType).includes("m4a"))
      ext = "m4a";
    const file = await toFile(buf, `recording.${ext}`, { type: mimeType });

    const body = { file, model: "whisper-1" };
    if (language === "hi") body.language = "hi";

    const tr = await openai.audio.transcriptions.create(body);
    const text = (tr.text || "").trim();
    return res.json({ transcript: text });
  } catch (e) {
    return res
      .status(500)
      .json({ error: "Transcription failed: " + e.message });
  }
});
...
app.get("/api/outbreak-check", async (req, res) => {
  try {
    ...
    const last7 = logs.filter(
      (l) => Date.now() - new Date(l.timestamp) < 7 * 86400000,
    );
    if (last7.length < 3)
      return res.json({
        alerts: [],
        message: "Not enough data for outbreak detection.",
      });
...
app.post("/api/chat", async (req, res) => {
  ...
  try {
    ...
    const aiResponse = await generateASHAResponse(
      query,
      symptoms,
      ranked,
      language,
    ...
    console.log(
      `[Chat] Done — triage:${triageResult.level}(${triageResult.score}) articles:${ranked.length}`,
    );
    ...
    return res
      .status(500)
      .json({ error: err.message || "AI service unavailable. Please retry." });
  }
});
    ...
    (l) =>
      ...
      l.symptoms.some(
        (s) =>
          ...
          s.toLowerCase().includes("bukhar"),
      ),
  );
  if (!feverLogs.length) return null;
  const earliest = Math.min(
    ...feverLogs.map((l) => new Date(l.timestamp).getTime()),
  );
...
app.get("/api/analytics", async (req, res) => {
  try {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      dailyTrend.push({
        date: day,
        count: recent.filter((l) => l.timestamp?.startsWith(day)).length,
      });
    }
    ...
    recent.forEach((l) => {
      if (l.triageLevel)
        triageSummary[l.triageLevel] = (triageSummary[l.triageLevel] || 0) + 1;
    });
    ...
    const feverAlerts = patientIds
      .map((pid) => checkFeverFollowup(logs, pid))
      .filter(Boolean);
    ...
    return res.json({
      ...
      avgTriageScore: recent.length
        ? Math.round(
            recent.reduce((s, l) => s + (l.triageScore || 50), 0) /
              recent.length,
          )
        : 0,
    });
...
// Static HTML/CSS/JS (after API routes)
